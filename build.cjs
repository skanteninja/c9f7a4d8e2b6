...
const assetVersion = '0.10.14-royal-maple-medallion-frames';
...
function patchApp(raw) {
  ...
  // Never render a third-party character fallback. The same-origin base
  // character route is sufficient and keeps the public site asset-owned.
  app = app.replace(
...
const royalMapleFrameAsset = path.join(source, 'assets', 'royal-maple-card-frame.svg');
if (fs.existsSync(royalMapleFrameAsset)) {
  fs.mkdirSync(path.join(out, 'assets'), { recursive: true });
  fs.copyFileSync(royalMapleFrameAsset, path.join(out, 'assets', 'royal-maple-card-frame.svg'));
}
