...
(() => {
  ...
  function renderSkills(){
    if(activeBuild()?.id==='warrior-fighter'){
      ...
      const cards=names.map(name=>`<article class="skill-beginner-card fighter-skill-card"><span class="skill-beginner-icon">${skillImgTag(name,'skill-icon')}</span><div><div class="skill-beginner-title"><b>${esc(name)}</b><span>Classic</span></div><p>${esc(D.skillIcons[name].desc||'Fighter skill')}</p><small class="skill-beginner-meta">${esc(D.skillIcons[name].role||'Combat skill')}</small></div></article>`).join('');
      const rows=D.skills.map(row=>`<div class="skill-row ${Number(row.Level)===state.level?'current':''}" data-informational="1"><b style="color:#8bdfff">Lv${esc(row.Level)}</b><b>${esc(row.SP)}</b><div class="skill-icon-host">${skillImgTag(String(row.Spend).split(/[+,]/)[0].trim())}</div><div class="skill-spend">${esc(row.Spend)}</div><div class="skill-why">${esc(row['Why This Is The Action']||'Follow the Fighter progression.')}</div><div><span class="beta-tag">verify</span></div><div class="skill-result" style="grid-column:4/-1">${esc(row['Result After Level']||'')}</div></div>`).join('');
      document.getElementById('skill-list').innerHTML=`<section class="skill-beginner-reference"><div class="skill-beginner-head"><div><span class="eyebrow">FIGHTER · LV1–70</span><h3>Fighter Skills</h3></div><p>Sword-focused Classic route. Current checkpoint: ${esc(current?.Spend||'Follow the level plan.')}</p></div><div class="skill-beginner-grid">${cards}</div></section>${rows}`;
      return;
    }
