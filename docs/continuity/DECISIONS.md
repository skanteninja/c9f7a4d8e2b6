...
## 2026-09-17 — Royal Maple reference fidelity is visible in the rendered dashboard

- The supplied Royal Maple equipment-card reference is the visual truth for dashboard card styling, not merely a color palette.
- Keep the existing dashboard layout and compact slot geometry, but make the visual treatment unmistakable: dark plum/parchment surfaces, visible antique-gold double frames, maple-leaf ornaments, stronger gold slot borders, and readable ivory/serif headings.
- The class-town artwork may show through the Equipment Inventory card as a controlled visual layer; it must remain darkened enough for item labels, icons, and controls to stay readable.
- Remove generic teal/cyan corner marks when they conflict with the Royal Maple reference. Reuse the same red maple-leaf asset for the slot ornaments and keep interactions/data unchanged.

## 2026-09-17 — Royal Maple card treatment applies to dashboard cards

- Treat the dashboard’s outer panels and the actionable cards nested inside them as part of the approved Royal Maple visual system, not as untouched legacy surfaces.
- Use the reference treatment’s antique-gold double frame, square corners, dark plum/parchment surface, and restrained corner ornament while preserving the established grid and card heights.
- Keep town imagery as a page-level layer; cards should remain readable, self-contained Royal Maple surfaces rather than becoming transparent image panels.
...
- The Continue/Reset choice is a per-browser-tab entry decision, not a per-refresh decision.
- Store the last active build and its session-flow stage in `sessionStorage`: a completed stage suppresses the prompt on refresh, a different build begins a new decision, and a new tab has no prior stage.
- Keep `pending` and `gender` states blocking until the user completes the required action; reset must return to gender selection without allowing an incomplete flow to be silently bypassed.
...
- The earlier live baseline was 0.10.3-session-focus-trap. The current live checkpoint is 0.10.4-session-entry-once, generated site commit ea2aaf0e88508437afddd8db446f1b711103f077, with Verify Live run 35092685632 passed after the asset-token refresh.
