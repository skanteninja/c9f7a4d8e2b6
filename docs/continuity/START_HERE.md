...
The current live checkpoint is `0.10.3-session-focus-trap`. A saved build asks whether to Continue or Reset; Reset requires a second confirmation with a loss summary. Fresh and reset sessions require Male/Female selection, and the selected gender is persisted per build, reflected in the Classic avatar, and applied to gender-locked equipment filtering. Session dialogs now contain keyboard focus, and the refreshed live asset token was verified after deployment. Verify Live `34999212897` passed after Cloud Browser QA exercised the full flow.
