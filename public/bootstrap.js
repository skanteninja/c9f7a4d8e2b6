(()=>{
  ...
  function loadEnhancement(){
    if(!document.querySelector('link[data-monster-integrity]')){const link=document.createElement('link');link.rel='stylesheet';link.href=`monster-integrity.css?v=${V}`;link.dataset.monsterIntegrity='1';document.head.appendChild(link)}
    const loadIntegrity=()=>{if(document.querySelector('script[data-monster-integrity]'))return;const ext=document.createElement('script');ext.src=`monster-integrity.js?v=${V}`;ext.defer=true;ext.dataset.monsterIntegrity='1';document.body.appendChild(ext)};
    if(window.TCW_MONSTER_DROP_EVIDENCE){loadIntegrity();return}
    if(document.querySelector('script[data-monster-drop-evidence]'))return;
    const evidence=document.createElement('script');evidence.src=`monster-drop-evidence.js?v=${V}`;evidence.defer=true;evidence.dataset.monsterDropEvidence='1';evidence.onload=loadIntegrity;evidence.onerror=loadIntegrity;document.body.appendChild(evidence)
  }
