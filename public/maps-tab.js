(() => {
  ...
  const LAYOUT_INDEX='/map-layouts.json';
  ...
  const VICTORIA_MARKERS=[
    ...
    ['010005000',406.5,387,'town'],['010001030',291,480,'spot'],['010001020',310.5,526.5,'spot'],['010001010',355.5,558,'spot'],['010001000',436.5,562.5,'town'],['010001090',427.5,504,'spot'],['010001100',414,471,'spot'],['010001060',513,555,'spot'],['010001070',528,526.5,'spot'],['010001080',537,487.5,'spot'],
  ...
  const CONTINENTS=[
    ...
    {id:'victoria',name:'Victoria Island',subtitle:'Henesys · Ellinia · Perion · Kerning · Sleepywood',image:'/game-media/worldmap/victoria-island.webp',mark:'',currentMarkers:true,aliases:'victoria henesys ellinia perion kerning lith sleepywood dungeon ant tunnel florina nautilus'},
  ...
  const ATLAS_CURRENT_ID_ALIASES={'100000000':'010001000','105040300':'010005000'};
  const ATLAS_CURRENT_NAME_ALIASES={'100000000':'henesys'};
  async function loadAtlas(){
    if(state.atlas)return state.atlas;if(state.atlasLoading)return state.atlasLoading;
    state.atlasLoading=fetch(`${ATLAS_ROOT}data/markers.json`,{cache:'force-cache'}).then(r=>{if(!r.ok)throw Error('atlas markers '+r.status);return r.json();}).then(x=>{
      state.atlas=x||{};
      // The original overview has a dungeon-cluster point, but omits the town.
      // Give that entrance a direct destination to the NPC-populated city.
      const dungeon=state.atlas.victoria?.markers?.find(p=>p.label==='Victoria Island Dungeon');
      if(dungeon){dungeon.maps=[105040300,...dungeon.maps];dungeon.label='Sleepywood';}
      return state.atlas;
    }).catch(()=>{state.atlas={};return state.atlas;});return state.atlasLoading;
  }
  function atlasMapFor(point){for(const raw of point?.maps||[]){const digits=String(raw??'').replace(/\D/g,''),id=padMap(digits),numeric=String(Number(digits)),alias=ATLAS_CURRENT_ID_ALIASES[numeric];if(alias){const currentAlias=state.byId.get(alias)||state.current.find(m=>padMap(m.id)===alias);if(currentAlias)return state.byRef.get(refFor(currentAlias))||currentAlias;}const aliasName=ATLAS_CURRENT_NAME_ALIASES[numeric];if(aliasName){const currentNamed=state.current.find(m=>norm(m.name)===aliasName);if(currentNamed)return state.byRef.get(refFor(currentNamed))||currentNamed;}const direct=state.byRef.get(`l:${id}`)||state.byRef.get(`l:${numeric}`);if(direct)return direct;const cur=state.byId.get(id);if(cur)return cur;const any=state.maps.find(m=>String(Number(String(m.id||'').replace(/\D/g,'')))===numeric||(m.legacy_ids||[]).some(x=>String(Number(String(x).replace(/\D/g,'')))===numeric));if(any)return any;}const label=norm(point?.label||'');if(label){const current=state.current.find(m=>norm(m.name)===label);if(current)return state.byRef.get(refFor(current))||current;const legacy=state.legacy.find(m=>norm(m.name)===label);if(legacy)return state.byRef.get(refFor(legacy))||legacy;}return null;}
  ...
  function setAtlasReady(){const img=q('#maps-full-world-image'),meta=state.atlas?.world,pointCount=q('#maps-continent-hotspots')?.querySelectorAll('.maps-atlas-world-point').length||0,requiredPoints=CONTINENTS.filter(c=>mapsForContinent(c.id).length).length;if(img?.complete&&img.naturalWidth>200&&meta?.markers?.length>=10&&requiredPoints>0&&pointCount>=requiredPoints)document.documentElement.classList.add('maps-atlas-visual-ready');}
  function renderWorldAtlasMarkers(){
    const host=q('#maps-continent-hotspots'),meta=state.atlas?.world;if(!host)return;host.innerHTML='';
    for(const c of CONTINENTS){
      if(!mapsForContinent(c.id).length)continue;
      const point=meta?.markers?.find(p=>atlasContinentFor(p)===c.id);
      const fallback=WORLD_HOTSPOTS[c.id]||[86,88];
      const b=document.createElement('button');b.type='button';b.className='maps-atlas-world-point major sprite-fallback';b.dataset.continent=c.id;
      b.style.left=`${point?point.x/meta.width*100:fallback[0]}%`;b.style.top=`${point?point.y/meta.height*100:fallback[1]}%`;
      b.title=`Open ${c.name}`;b.setAttribute('aria-label',`Open ${c.name}`);b.innerHTML=`<span>${esc(c.name)}</span>`;
      b.addEventListener('click',()=>openContinent(c.id));host.appendChild(b);
    }setAtlasReady();
  }
  function renderAtlasSheetMarkers(id){const host=q('#maps-sheet-markers'),meta=state.atlas?.[id];if(!host||!meta?.markers?.length)return;host.innerHTML='';for(const point of meta.markers){const maps=[...new Set((point.maps||[]).map(raw=>atlasMapFor({maps:[raw]})).filter(m=>m&&m.__continent===id))];const map=maps[0];if(!map)continue;const label=point.label||map.name;const b=document.createElement('button');b.type='button';b.className=`atlas-map-point type-${point.type}${map?'':' unresolved'}`;if(map)b.dataset.mapId=padMap(map.id);b.style.left=`${point.x/meta.width*100}%`;b.style.top=`${point.y/meta.height*100}%`;b.title=map?map.name:label;b.innerHTML=`<img src="${atlasMarkerUrl(point.type)}" alt=""><span>${esc(label)}</span>`;const icon=b.querySelector('img');icon?.addEventListener('error',()=>{icon.remove();b.classList.add('sprite-fallback')});b.addEventListener('click',()=>{if(maps.length>1){showAtlasGroup(label,maps);return;}if(map){showDetail(map.__ref,'continent',id);return;}openExplorer();const search=q('#maps-explorer-search');if(search){search.value=label;renderExplorer();}});host.appendChild(b);}document.documentElement.classList.add('maps-sheet-markers-ready');}
  function showAtlasGroup(label,maps){
    const host=q('#maps-continent-list');renderMapRows(host,maps,maps.length);
    q('#maps-continent-count').textContent=`${label} · ${maps.length} maps`;
    const reset=document.createElement('button');reset.type='button';reset.className='btn';reset.textContent='Show all maps in this region';
    reset.addEventListener('click',()=>renderContinentList());host.prepend(reset);host.scrollIntoView({block:'nearest'});
  }
  ...
  async function loadData(){
    ...
    const legacyFetch=Promise.resolve([]);
    state.loading=Promise.all([
      ...
      fetch(`${DATA_ROOT}map_manifest.json`,{cache:'force-cache'}).then(r=>{if(!r.ok)throw Error('Map manifest unavailable');return r.json();}),legacyFetch,
      fetch(LAYOUT_INDEX,{cache:'no-cache'}).then(r=>{if(!r.ok)throw Error('Verified map index unavailable');return r.json();})
    ]).then(([maps,lookups,portals,monsters,mapManifest,legacy,layouts])=>{
      if(!layouts.current||!layouts.legacy)throw Error('Invalid verified map index');
      state.layouts=layouts;state.mapManifest=mapManifest;
      state.current=flattenMaps(maps).filter(m=>layouts.current[padMap(m.id)]===mapManifest[padMap(m.id)]&&Boolean(mapManifest[padMap(m.id)]));
      // The public atlas is intentionally limited to OSMS/Classic records.
      // Legacy GMS v83 rows are retained only as private source material.
      const oldMaps=[];
      state.legacy=[];
      ...
      // Retain old-ID aliases even when the old renderer is unusable.
      for(const old of oldMaps){const hit=currentKeys.get(identityKey(old));if(hit){hit.legacy_ids=hit.legacy_ids||[];if(!hit.legacy_ids.includes(String(old.id)))hit.legacy_ids.push(String(old.id));hit.legacy=true;}}
      ...
      for(const m of state.current)for(const id of m.legacy_ids||[])state.byRef.set(`l:${id}`,m);
      for(const [oldRef,currentRef] of Object.entries(layouts.aliases||{})){const target=state.byRef.get(currentRef);if(target)state.byRef.set(oldRef,target);}
      const filter=q('#maps-explorer-continent');if(filter)filter.innerHTML='<option value="all">All continents</option>'+CONTINENTS.filter(c=>mapsForContinent(c.id).length).map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join('');state.loaded=true;document.documentElement.classList.add('maps-data-ready');if(state.legacy.length)document.documentElement.classList.add('maps-legacy-data-ready');
      return state;
  ...
  function mapImage(map){const key=padMap(map?.id);if(map.source==='current'){const hash=state.layouts?.current[key];return hash?`/game-data/data/maps/${hash}.webp`:'';}return state.layouts?.legacy[key]?`/game-media/legacy-map/${key}/render`:'';}
  ...
  function sourceLabel(){return 'OSMS / CLASSIC BETA';}
  function sourceClass(){return 'current';}

  function ensureShell(){
    const root=page();if(!root||root.dataset.mapsShell==='2')return;root.dataset.mapsShell='2';
    root.innerHTML=`
      <div class="maps-hero">
        <div><span class="eyebrow">MAPLE WORLD ATLAS</span><h2>Maps</h2><p>Browse the OSMS Classic beta world, drill into a continent, or search the verified map database.</p></div>
        <a class="maps-audit-link" href="/map-audit.html">Map review &amp; removal list</a><div class="maps-view-switch" role="tablist"><button type="button" data-map-view="world" class="active">World</button><button type="button" data-map-view="explorer">All maps</button></div>
      </div>
      ...
      <div id="maps-world" class="maps-view active">
        <section class="maps-full-world panel">
          <div class="maps-overview-head"><div><b>Maple World</b><span id="maps-world-count">Loading OSMS Classic beta maps…</span></div><button type="button" id="maps-open-explorer" class="mini-btn">Search every map</button></div>
  ...
  function updateBreadcrumb(){const back=q('#maps-local-back'),crumb=q('#maps-crumbs');if(!back||!crumb)return;if(state.view==='world'){back.hidden=true;crumb.innerHTML='<b>Maple World</b><span>Choose a continent or open the full map database.</span>';}else if(state.view==='continent'){back.hidden=false;const c=continentConfig(state.continent);back.textContent='← Maple World';crumb.innerHTML=`<b>Maple World › ${esc(c.name)}</b><span>${esc(c.subtitle)}</span>`;}else if(state.view==='explorer'){back.hidden=false;back.textContent='← Maple World';crumb.innerHTML='<b>Maple World › All Maps</b><span>Search all available OSMS / Classic beta maps.</span>';}else{back.hidden=false;const map=state.selected&&state.byRef.get(state.selected);const label=state.returnView==='continent'?continentConfig(state.returnContinent).name:state.returnView==='explorer'?'All Maps':'Maple World';back.textContent=`← ${label}`;crumb.innerHTML=`<b>Maple World › ${esc(label)} › ${esc(map?.name||'Map')}</b><span>Escape returns one level.</span>`;}}
  ...
  function renderWorld(){
    setView('world');document.documentElement.classList.remove('maps-atlas-visual-ready');const count=q('#maps-world-count');if(count)count.textContent=`${state.current.length} verified OSMS / Classic beta maps`;
    const grid=q('#maps-continent-grid'),hot=q('#maps-continent-hotspots');if(!grid||!hot)return;grid.innerHTML='';hot.innerHTML='';
    for(const c of CONTINENTS){const rows=mapsForContinent(c.id);if(!rows.length)continue;const current=rows.filter(m=>m.source==='current').length,legacy=rows.filter(m=>m.source==='legacy').length;const card=document.createElement('button');card.type='button';card.className='maps-continent-card';card.dataset.continent=c.id;card.innerHTML=`${continentArt(c)}<span class="maps-continent-copy"><b>${esc(c.name)}</b><small>${esc(c.subtitle)}</small><em><strong>${rows.length}</strong> maps${current?` · ${current} current`:''}${legacy?` · ${legacy} legacy-only`:''}</em></span>`;card.querySelectorAll('img').forEach(im=>im.addEventListener('error',()=>{im.remove();card.classList.add('maps-continent-art-missing')}));card.addEventListener('click',()=>openContinent(c.id));grid.appendChild(card);}
  ...
  function renderVictoriaMarkers(){const host=q('#maps-sheet-markers');if(!host)return;host.innerHTML='';for(const [id,x,y,type] of VICTORIA_MARKERS){const map=state.byId.get(id);if(!map)continue;const name=map.name;const btn=document.createElement('button');btn.type='button';btn.className=`world-map-marker ${type}`;btn.dataset.mapId=id;btn.style.left=`${x/WORLD_W*100}%`;btn.style.top=`${y/WORLD_H*100}%`;btn.title=name;btn.setAttribute('aria-label',`Open ${name}`);const kind=type==='cluster'?'cluster':type==='town'?'town':'spot';btn.innerHTML=`<img src="/game-media/worldmap/marker-${kind}.png" alt="">${type==='town'?`<span>${esc(name)}</span>`:''}`;const im=btn.querySelector('img');im?.addEventListener('error',()=>{im.remove();btn.classList.add('sprite-fallback')});btn.addEventListener('click',()=>showDetail(map?map.__ref:`c:${id}`,'continent','victoria'));host.appendChild(btn);}document.documentElement.classList.toggle('victoria-world-markers-ready',host.children.length>=50);}
  ...
  function renderContinentList(){const host=q('#maps-continent-list');if(!host)return;const value=norm(q('#maps-continent-search')?.value||'');let rows=mapsForContinent(state.continent);if(value)rows=rows.filter(m=>m.__search.includes(value));renderMapRows(host,rows,rows.length);const count=q('#maps-continent-count');if(count)count.textContent=`${rows.length} / ${mapsForContinent(state.continent).length} maps`;}
  function openContinent(id,push=true){if(!mapsForContinent(id).length){openWorld();return;}state.continent=id;const c=continentConfig(id);setView('continent');q('#maps-continent-title').textContent=c.name;q('#maps-continent-subtitle').textContent=c.subtitle;q('#maps-continent-kicker').textContent='OSMS / CLASSIC BETA';const img=q('#maps-sheet-image');img.src=c.image||`${ATLAS_ROOT}sheets/maple-world.png`;img.alt=`${c.name} world map`;const stage=q('#maps-sheet-stage');stage.classList.toggle('victoria-sheet',id==='victoria');const note=q('#maps-sheet-note');if(id==='victoria'&&c.currentMarkers){renderVictoriaMarkers();note.innerHTML='<span><i class="town"></i>Town</span><span><i class="spot"></i>Field / route</span><span><i class="cluster"></i>Dungeon cluster</span><small>Beta Victoria Island · click a town or route. Every map is listed below.</small>';}else{q('#maps-sheet-markers').innerHTML='';loadAtlas().then(()=>renderAtlasSheetMarkers(id));note.innerHTML='<small>Click a travel point to open its map or choose from its connected maps. Every available map is listed below.</small>';}q('#maps-continent-search').value='';renderContinentList();page()?.scrollIntoView({block:'start'});document.documentElement.classList.add('maps-continent-ready');}
  ...
  function renderExplorer(){const rows=explorerMatches(),host=q('#maps-explorer-list');if(!host)return;renderMapRows(host,rows,state.explorerLimit);q('#maps-explorer-total').textContent=String(state.maps.length);const status=q('#maps-explorer-status');if(status)status.textContent=`Showing ${Math.min(rows.length,state.explorerLimit)} of ${rows.length} matching maps · ${state.current.length} OSMS / Classic beta maps`;const more=q('#maps-explorer-more');if(more){more.hidden=rows.length<=state.explorerLimit;more.textContent=`Show more (${Math.max(0,rows.length-state.explorerLimit)} remaining)`;}document.documentElement.classList.add('maps-explorer-ready');}
  ...
  async function showDetail(ref,returnView=state.view,returnContinent=state.continent){await loadData();let map=state.byRef.get(ref);if(!map&&String(ref).startsWith('c:'))map=state.byId.get(padMap(String(ref).slice(2)));if(!map){openExplorer();const status=q('#maps-explorer-status');if(status)status.innerHTML='This map was removed because a complete layout could not be verified. <a href="/map-audit.html">See map review results</a>.';return;}state.returnView=returnView==='detail'?state.returnView:returnView;state.returnContinent=returnContinent;renderDetail(map);page()?.scrollIntoView({block:'start'});}
  ...
  function bindShell(){
    ...
    q('#maps-explorer-more')?.addEventListener('click',()=>{const oldCount=q('#maps-explorer-list').children.length;state.explorerLimit+=120;renderExplorer();q('#maps-explorer-list').children[oldCount]?.scrollIntoView({block:'start',behavior:'smooth'});});page()?.querySelectorAll('[data-map-keyword]').forEach(b=>b.addEventListener('click',()=>{q('#maps-explorer-search').value=b.dataset.mapKeyword;state.explorerLimit=120;renderExplorer();}));
  ...
  async function initMaps(){ensureShell();try{const initial=!state.loaded;await loadData();if(initial)renderWorld();document.documentElement.classList.add('maps-tab-ready');}catch(err){const count=q('#maps-world-count');if(count)count.textContent='Map data could not be loaded.';console.error('Maps tab:',err);}}
  async function openMaps(){document.querySelectorAll('.page').forEach(x=>x.classList.toggle('active',x===page()));document.querySelectorAll('#nav .nav-btn').forEach(x=>x.classList.toggle('active',x.dataset.page==='maps'));const title=document.getElementById('page-title'),sub=document.getElementById('page-subtitle'),back=document.getElementById('page-back');if(title)title.textContent='Maps';if(sub)sub.textContent='Maple World → continent → map, plus a searchable current + old-school map database.';if(back)back.hidden=true;window.scrollTo(0,0);await initMaps();}
  ...
  async function restore(route,controls){await openMaps();if(route.view==='continent'&&route.continent)openContinent(route.continent);else if(route.view==='explorer')openExplorer();else if(route.view==='detail'&&route.selected)await showDetail(route.selected,route.returnView||'explorer',route.returnContinent);else openWorld();for(const [id,value] of Object.entries(controls)){const el=document.getElementById(id);if(el)el.value=value;}state.explorerLimit=route.explorerLimit||120;if(route.view==='explorer')renderExplorer();if(route.view==='continent')renderContinentList();}
  ensureNav();ensureShell();initMaps();new MutationObserver(()=>{ensureNav();ensureShell();}).observe(document.body,{childList:true,subtree:true});window.TCW_MAPS={restore,showDetail,open:openMaps,openWorld,openContinent,openExplorer,goBack,get state(){return state;}};
})();
