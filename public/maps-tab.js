(() => {
  ...
  function setAtlasReady(){const img=q('#maps-full-world-image'),meta=state.atlas?.world,pointCount=q('#maps-continent-hotspots')?.querySelectorAll('.maps-atlas-world-point').length||0;if(img?.complete&&img.naturalWidth>200&&meta?.markers?.length>=10&&pointCount>=10)document.documentElement.classList.add('maps-atlas-visual-ready');}
