(() => {
  ...
  const start=setInterval(async()=>{if(!window.TCW_NAV||!window.TCW_MAPS?.state.loaded)return;clearInterval(start);const u=new URL(location.href);let s=history.state;
    if(!s?.tcw&&(u.searchParams.has('page')||u.searchParams.has('build')))s={tcw:true,build:u.searchParams.get('build')||selectedBuild(),page:u.searchParams.get('page')||'dashboard',map:{view:u.searchParams.get('view')||'world',continent:u.searchParams.get('continent'),selected:u.searchParams.get('map'),returnView:'explorer'},controls:Object.fromEntries(controlIds.filter(k=>u.searchParams.has(k)).map(k=>[k,u.searchParams.get(k)]))};
