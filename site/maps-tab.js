(() => {
  ...
  function renderVictoriaMarkers(){const host=q('#maps-sheet-markers');if(!host)return;host.innerHTML='';for(const [id,x,y,type] of VICTORIA_MARKERS){const map=state.byId.get(id);if(!map)continue;const name=map.name;const btn=document.createElement('button');btn.type='button';btn.className=`world-map-marker ${type}`;btn.dataset.mapId=id;btn.style.left=`${x/WORLD_W*100}%`;btn.style.top=`${y/WORLD_H*100}%`;btn.title=`${name} · #${id}`;btn.setAttribute('aria-label',`Open ${name}`);const kind=type==='cluster'?'cluster':type==='town'?'town':'spot';btn.innerHTML=`<img src="${ATLAS_ROOT}markers/current-${kind}.png" alt="">${type==='town'?`<span>${esc(name)}</span>`:''}`;const im=btn.querySelector('img');im?.addEventListener('error',()=>{im.remove();btn.classList.add('sprite-fallback')});btn.addEventListener('click',()=>showDetail(map?map.__ref:`c:${id}`,'continent','victoria'));host.appendChild(btn);}document.documentElement.classList.toggle('victoria-world-markers-ready',host.children.length>=50);}
  ...
  function renderExplorer(){const rows=explorerMatches(),host=q('#maps-explorer-list');if(!host)return;renderMapRows(host,rows,state.explorerLimit);q('#maps-explorer-total').textContent=String(state.maps.length);const status=q('#maps-explorer-status');if(status)status.textContent=`Showing ${Math.min(rows.length,state.explorerLimit)} of ${rows.length} matching maps · ${state.current.length} current Classic + ${state.legacy.length} old-school records before deduping`;const more=q('#maps-explorer-more');if(more){more.hidden=rows.length<=state.explorerLimit;more.textContent=`Show more (${Math.max(0,rows.length-state.explorerLimit)} remaining)`;}document.documentElement.classList.add('maps-explorer-ready');}
