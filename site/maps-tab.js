(() => {
    ...
    root.innerHTML=`
      ...
      <div id="maps-world" class="maps-view active">
        <section class="maps-full-world panel">
          ...
          <div class="maps-full-world-stage"><img id="maps-full-world-image" src="/game-media/worldmap-legacy/maple-world.png" alt="Maple World"><div id="maps-continent-hotspots"></div></div>
  ...
  function renderWorld(){setView('world');const count=q('#maps-world-count');if(count)count.textContent=`${state.current.length} current Classic records · ${state.legacy.length} old-school records · ${state.maps.length} merged maps`;const grid=q('#maps-continent-grid'),hot=q('#maps-continent-hotspots');if(!grid||!hot)return;grid.innerHTML='';hot.innerHTML='';for(const c of CONTINENTS){const rows=mapsForContinent(c.id);if(!rows.length&&c.id==='other')continue;const current=rows.filter(m=>m.source==='current').length;const legacy=rows.filter(m=>m.source==='legacy').length;const card=document.createElement('button');card.type='button';card.className='maps-continent-card';card.dataset.continent=c.id;card.innerHTML=`<span class="maps-continent-thumb">${c.image?`<img src="${c.image}" alt="" loading="lazy">`:''}</span><span><b>${esc(c.name)}</b><small>${esc(c.subtitle)}</small><em>${rows.length} maps${current?` · ${current} current`:''}${legacy?` · ${legacy} legacy-only`:''}</em></span>`;card.addEventListener('click',()=>openContinent(c.id));grid.appendChild(card);const pos=WORLD_HOTSPOTS[c.id];if(pos){const h=document.createElement('button');h.type='button';h.className='maps-continent-hotspot';h.style.left=`${pos[0]}%`;h.style.top=`${pos[1]}%`;h.innerHTML=`<b>${esc(c.name)}</b><small>${rows.length}</small>`;h.addEventListener('click',()=>openContinent(c.id));hot.appendChild(h);}}
    document.documentElement.classList.add('maps-world-ready');
  }

  function renderVictoriaMarkers(){const host=q('#maps-sheet-markers');if(!host)return;host.innerHTML='';for(const [id,x,y,type] of VICTORIA_MARKERS){const map=state.byId.get(id),name=map?.name||`Map #${id}`;const btn=document.createElement('button');btn.type='button';btn.className=`world-map-marker ${type}`;btn.dataset.mapId=id;btn.style.left=`${x/WORLD_W*100}%`;btn.style.top=`${y/WORLD_H*100}%`;btn.title=`${name} · #${id}`;btn.setAttribute('aria-label',`Open ${name}`);btn.innerHTML=`<img src="/game-media/worldmap/marker-${type==='cluster'?'cluster':type==='town'?'town':'spot'}.png" alt="">${type==='town'?`<span>${esc(name)}</span>`:''}`;btn.addEventListener('click',()=>showDetail(map?map.__ref:`c:${id}`,'continent','victoria'));host.appendChild(btn);}document.documentElement.classList.toggle('victoria-world-markers-ready',host.children.length>=50);}
  function renderMapRows(host,rows,limit=180){host.innerHTML='';for(const map of rows.slice(0,limit)){const b=document.createElement('button');b.type='button';b.className='maps-db-row';b.dataset.mapRef=map.__ref;b.innerHTML=`<span class="maps-db-main"><b>${esc(map.name)}</b><small>${esc(map.street_name||continentConfig(map.__continent).name)}</small></span><span class="map-source-pill ${sourceClass(map)}">${sourceLabel(map)}</span><code>#${esc(String(map.id))}</code>`;b.addEventListener('click',()=>showDetail(map.__ref,state.view,state.continent));host.appendChild(b);}}
  ...
  function openContinent(id,push=true){state.continent=id;const c=continentConfig(id);setView('continent');q('#maps-continent-title').textContent=c.name;q('#maps-continent-subtitle').textContent=c.subtitle;q('#maps-continent-kicker').textContent=id==='victoria'?'CURRENT CLASSIC + OLD SCHOOL':'CURRENT CLASSIC + OLD-SCHOOL INDEX';const img=q('#maps-sheet-image');img.src=c.image||'/game-media/worldmap-legacy/maple-world.png';img.alt=`${c.name} world map`;const stage=q('#maps-sheet-stage');stage.classList.toggle('victoria-sheet',id==='victoria');const note=q('#maps-sheet-note');if(id==='victoria'){renderVictoriaMarkers();note.innerHTML='<span><i class="town"></i>Town</span><span><i class="spot"></i>Field / route</span><span><i class="cluster"></i>Dungeon cluster</span><small>Victoria keeps the exact clickable Classic World markers.</small>';}else{q('#maps-sheet-markers').innerHTML='';note.innerHTML='<small>Old-school region sheet for orientation. Use the map list beside it to open the actual maps; current Classic records are preferred wherever available.</small>';}q('#maps-continent-search').value='';renderContinentList();page()?.scrollIntoView({block:'start'});document.documentElement.classList.add('maps-continent-ready');}
