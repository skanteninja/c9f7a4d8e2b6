(() => {
  ...
  async function loadData(){
    ...
    state.loading=Promise.all([
      ...
      // The public atlas is intentionally limited to TCW/Classic records.
  ...
  function sourceLabel(){return 'TCW / CLASSIC BETA';}
  ...
  function ensureShell(){
    ...
    root.innerHTML=`
      <div class="maps-hero">
        <div><span class="eyebrow">MAPLE WORLD ATLAS</span><h2>Maps</h2><p>Browse the TCW Classic beta world, drill into a continent, or search the verified map database.</p></div>
      ...
      <div id="maps-world" class="maps-view active">
        <section class="maps-full-world panel">
          <div class="maps-overview-head"><div><b>Maple World</b><span id="maps-world-count">Loading TCW Classic beta maps…</span></div><button type="button" id="maps-open-explorer" class="mini-btn">Search every map</button></div>
  ...
  function updateBreadcrumb(){const back=q('#maps-local-back'),crumb=q('#maps-crumbs');if(!back||!crumb)return;if(state.view==='world'){back.hidden=true;crumb.innerHTML='<b>Maple World</b><span>Choose a continent or open the full map database.</span>';}else if(state.view==='continent'){back.hidden=false;const c=continentConfig(state.continent);back.textContent='← Maple World';crumb.innerHTML=`<b>Maple World › ${esc(c.name)}</b><span>${esc(c.subtitle)}</span>`;}else if(state.view==='explorer'){back.hidden=false;back.textContent='← Maple World';crumb.innerHTML='<b>Maple World › All Maps</b><span>Search all available TCW / Classic beta maps.</span>';}else{back.hidden=false;const map=state.selected&&state.byRef.get(state.selected);const label=state.returnView==='continent'?continentConfig(state.returnContinent).name:state.returnView==='explorer'?'All Maps':'Maple World';back.textContent=`← ${label}`;crumb.innerHTML=`<b>Maple World › ${esc(label)} › ${esc(map?.name||'Map')}</b><span>Escape returns one level.</span>`;}}
  ...
  function renderWorld(){
    setView('world');document.documentElement.classList.remove('maps-atlas-visual-ready');const count=q('#maps-world-count');if(count)count.textContent=`${state.current.length} verified TCW / Classic beta maps`;
