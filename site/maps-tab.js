(() => {
  ...
  async function loadData(){
    ...
    state.loading=Promise.all([
      ...
      const filter=q('#maps-explorer-continent');if(filter)filter.innerHTML='<option value="all">All continents</option>'+CONTINENTS.filter(c=>mapsForContinent(c.id).length).map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join('');state.loaded=true;document.documentElement.classList.add('maps-data-ready','maps-current-only-ready');if(state.legacy.length)document.documentElement.classList.add('maps-legacy-data-ready');
      return state;
  ...
  function renderWorld(){
    setView('world');const count=q('#maps-world-count');if(count)count.textContent=`${state.current.length} verified TCW / Classic beta maps`;
  ...
  async function openMaps(){document.querySelectorAll('.page').forEach(x=>x.classList.toggle('active',x===page()));document.querySelectorAll('#nav .nav-btn').forEach(x=>x.classList.toggle('active',x.dataset.page==='maps'));const title=document.getElementById('page-title'),sub=document.getElementById('page-subtitle'),back=document.getElementById('page-back');if(title)title.textContent='Maps';if(sub)sub.textContent='Maple World → continent → map, plus a searchable Classic map catalog.';if(back)back.hidden=true;window.scrollTo(0,0);await initMaps();}
