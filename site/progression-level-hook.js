...
(() => {
  ...
  function ilTarget(level) {
    ...
    const stats={STR:4,DEX:4,INT:12,LUK:5};
    let action='Starting stats';
    for(let n=2;n<=lv;n++){
      let int=5,luk=0;
      if(n>=16&&n<=65){int=4;luk=1;}
      else if(n>=66&&n<=68){int=3;luk=2;}
      else if(n>=69){int=4;luk=1;}
      stats.INT+=int; stats.LUK+=luk;
      if(n===lv) action=luk?`+${int} INT / +${luk} LUK`:'+5 INT';
    }
