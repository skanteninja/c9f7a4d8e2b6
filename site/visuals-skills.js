(() => {
  ...
  const MAPLE_WZ='/game-media/skills/';
  ...
  async function enhanceDashboardSkillCards(idx){
    ...
    cards.forEach(card=>{
      ...
      let badge=card.querySelector('.visual-current-skill-badge');
      if(!badge){badge=document.createElement('span');badge.className='visual-current-skill-badge';card.appendChild(badge);}
